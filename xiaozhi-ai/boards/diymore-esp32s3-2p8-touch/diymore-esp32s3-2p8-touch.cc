...
#define TAG "DiymoreEsp32S3Touch"

// Minimal FT6336 capacitive-touch reader over I2C (shares the codec I2C bus).
class TouchDriver {
public:
...
class DiymoreEsp32S3Touch : public WifiBoard {
private:
    Button boot_button_;
    LcdDisplay *display_;
    ...
    static void TouchTask(void *arg) {
        auto *self = static_cast<DiymoreEsp32S3Touch*>(arg);
        ...
        while (true) {
            ...
            if (!t && down) {
                ...
                // long tap -> enter Wi-Fi config mode
                if (press > 3000) {
...
public:
    DiymoreEsp32S3Touch(): boot_button_(BOOT_BUTTON_GPIO)
    {
        InitializeI2c();
...
DECLARE_BOARD(DiymoreEsp32S3Touch);
